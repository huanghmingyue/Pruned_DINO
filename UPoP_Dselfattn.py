...
def update_alpha_parameters(model, p, pi, print_info):
    ...
    alpha_grad = torch.cat([alpha_grad_attn.view(-1), alpha_grad_ffn.view(-1), decoder_alpha_grad_self_attn.view(-1)])
    sorted_alpha_grad, indices = torch.sort(alpha_grad, descending=True)
    compression_weight = torch.ones_like(indices)
    compression_weight[indices < alpha_grad_all_attn.numel()] = 24
