...
def get_args_parser():
    ...
    parser.add_argument('--w_sp_dattn', default=1.28e-2, type=float, help='regularization coefficient for dattn')
    parser.add_argument('--p', default=0.5, type=float, help='total compression ratio')
    parser.add_argument('--interval', default=470, type=int, help='interval of updating compression mask')
    parser.add_argument('--lr-search', type=float, default=1e-4, help='search learning rate (default: 5e-4)')
    parser.add_argument('--epochs-search', default=16, type=int)
...
def main(args):
    ...
    # build model
    model, criterion, postprocessors = build_model_main(args, search=True)
    ...
    if args.resume:
        ...
        # finetune時要添加參數 strict=False 搜索模型加载到新模型#############################################################
    ...
    # ---------------------------------------------------------------------------------------------------------------------------------
    ######################## finetune ##########################################################################
    #search_config_path = r"./config/DINO/DINO_4scale.py" # change the path of the model config file
    #search_model_checkpoint_path = r"./checkpoint0015.pth" 
    #search_model, criterion_search, postprocessors_search = build_model_main(args, search=True)
    #search_checkpoint = torch.load(search_model_checkpoint_path)
    #search_model.load_state_dict(search_checkpoint['model'])
    #compress(model_without_ddp, search_model)
    #-----------------------------------------------------------------------------------------------------------------------------------
...
#-----------------------------------------------------------------------------------------------------
    ...
    for epoch in range(args.start_epoch, args.epochs_search):# !!!epochs改为epochs_search!!!!!!!!!!!!!!!!!!!
        epoch_start_time = time.time()
        if args.distributed:
            sampler_train.set_epoch(epoch)
        train_stats = train_one_epoch(# ----------加一个search=True-------------------------------
            model, criterion, data_loader_train, optimizer, device, epoch,
            args.clip_max_norm, wo_class_error=wo_class_error, lr_scheduler=lr_scheduler, args=args, logger=(logger if args.save_log else None), ema_m=ema_m, search = True)
        if args.output_dir:
    ...
    #-----------------------------------------------------------------------------------------------
    ########------finetune時注釋掉##################################################################
    print_compression_statistics(model)
    #-----------------------------------------------------------------------------------------------
